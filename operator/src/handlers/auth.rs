...
use crate::{
    ...
    kong_consumer, kong_plugin, patch_resource, Error, KupoPort,
};
...
async fn generate_api_key(crd: &KupoPort) -> Result<String, Error> {
    let namespace = crd.namespace().unwrap();
    let name = get_auth_name(&crd.name_any());
...
fn build_auth_plugin(crd: &KupoPort) -> Result<(ObjectMeta, JsonValue, JsonValue), Error> {
    let kong_plugin = kong_plugin();
...
fn build_host_key_plugin(crd: &KupoPort) -> Result<(ObjectMeta, JsonValue, JsonValue), Error> {
    let kong_plugin = kong_plugin();
...
fn build_acl_secret(crd: &KupoPort) -> Secret {
    ...
    let metadata = ObjectMeta {
        name: Some(get_acl_name(&crd.name_any())),
        owner_references: Some(vec![OwnerReference {
            api_version: KupoPort::api_version(&()).to_string(),
            kind: KupoPort::kind(&()).to_string(),
            name: crd.name_any(),
...
fn build_acl_plugin(crd: &KupoPort) -> Result<(ObjectMeta, JsonValue, JsonValue), Error> {
    let kong_plugin = kong_plugin();
...
fn build_consumer(crd: &KupoPort) -> Result<(ObjectMeta, JsonValue, JsonValue), Error> {
    let kong_consumer = kong_consumer();
    let config = get_config();
